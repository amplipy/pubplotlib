...
def add_to_ppt(prs, pngs):
    ...
    subtitle.txt = ""
    #remove all from slide
    while len(slide.shapes) > 0:
        for shape in slide.shapes:
            sp = shape._element
            sp.getparent().remove(sp)

    # Generate and add three figures for each slide
    for j in range(len(pngs)):
        
        # Define positions and size for each figure
        left = Inches(1)  # Position figures horizontally with some space
        top = Inches(j * 3 + 1)  # Keep the vertical position constant
        #width = Inches(2.5)  # Define a constant width
        #height = Inches(2)  # Define a constant height
        
        # Add figure to slide
        slide.shapes.add_picture(pngs[j], left, top)
